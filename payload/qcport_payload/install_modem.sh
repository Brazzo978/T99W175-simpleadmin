...
install_atcli_stack() {
    ...
    install_file "$FILES_DIR/systemd/atcli-perms.service" "/lib/systemd/system/atcli-perms.service" 644
    link_enable_service "atcli-perms.service"
    systemctl daemon-reload
    systemctl start atcli-perms.service >/dev/null 2>&1 || true
}
...
run_tests() {
    ...
    if command -v systemctl >/dev/null 2>&1; then
        systemctl status atcli-perms.service --no-pager >/dev/null 2>&1 || warn "atcli-perms.service status check failed"
