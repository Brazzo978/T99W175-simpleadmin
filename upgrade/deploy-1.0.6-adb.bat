...
set "SCRIPT_DIR=%~dp0"
set "PAYLOAD_ARCHIVE=%SCRIPT_DIR%simpleadmin-1.0.6-payload.tar.gz"
set "REMOTE_ARCHIVE=/tmp/simpleadmin-1.0.6-payload.tar.gz"
set "REMOTE_DIR=/tmp/simpleadmin-1.0.6-payload"
...
adb shell "cd /tmp && rm -rf %REMOTE_DIR% && tar -xzf %REMOTE_ARCHIVE% && sh %REMOTE_DIR%/upgrade-to-1.0.6.sh"
if errorlevel 1 exit /b 1
